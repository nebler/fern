...
document.getElementById('refresh').onclick=()=>load().catch(error=>show(error.message,true));load().catch(error=>show(error.message,true));try{const pending=readPendingSubmission();if(pending)void submitPending(pending,'Retrying the saved task command...')}catch(error){submit.disabled=true;show(error.message,true)}})();`
