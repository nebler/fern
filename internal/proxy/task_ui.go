...
const pendingSubmissionKey='fern.pending-task-submission.v1';
...
function readPendingSubmission(){let raw;try{raw=localStorage.getItem(pendingSubmissionKey)}catch{throw new Error('Durable browser storage is unavailable; no task was sent')}if(raw===null)return null;let value;try{value=JSON.parse(raw)}catch{throw new Error('The saved pending task is invalid; clear site data before continuing')}if(value?.version!==1||typeof value.idempotencyKey!=='string'||!/^[A-Za-z0-9._~-]{1,128}$/.test(value.idempotencyKey)||typeof value.body!=='string'||value.body.length>400000||Object.keys(value).sort().join(',')!=='body,idempotencyKey,version')throw new Error('The saved pending task is invalid; clear site data before continuing');return value}
function storePendingSubmission(value){const raw=JSON.stringify(value);try{localStorage.setItem(pendingSubmissionKey,raw);if(localStorage.getItem(pendingSubmissionKey)!==raw)throw new Error()}catch{throw new Error('The task could not be saved for reliable retry; no task was sent')}}
async function sendPendingSubmission(value){const path='/fern/api/v1/tasks',token=await csrf(path),response=await fetch(path,{method:'POST',headers:{'Content-Type':'application/json','Idempotency-Key':value.idempotencyKey,'X-Fern-CSRF-Token':token},body:value.body}),data=await response.json().catch(()=>null);if(!response.ok)throw new Error(data?.error?.message||'The task remains saved for retry');try{localStorage.removeItem(pendingSubmissionKey)}catch{}return data}
async function preview(path){const token=await csrf(path),response=await fetch(path,{method:'POST',headers:{Accept:'application/json','Content-Type':'application/json','X-Fern-CSRF-Token':token},body:'{}'}),data=await response.json().catch(()=>null);if(!response.ok)throw new Error(data?.error?.message||'Snapshot preview failed');return data}
...
async function sealTask(id){const snapshot=await preview('/fern/api/v1/tasks/'+encodeURIComponent(id)+'/seal-preview'),summary=snapshot.outcome==='no_changes'?'no changes':snapshot.manifestEntries+' changed path'+(snapshot.manifestEntries===1?'':'s');if(!confirm('Seal commit '+snapshot.resultCommit.slice(0,12)+' with '+summary+'? This records your authorization, not OpenCode success.'))return;await mutation('/fern/api/v1/tasks/'+encodeURIComponent(id)+'/seal',snapshot);show('Exact snapshot authorized for sealing.');await load()}
function progressFor(value){if(value.publication)return 'publication '+value.publication.state.replaceAll('_',' ');const verification=value.verifications?.at(-1);if(verification)return 'verification '+verification.state.replaceAll('_',' ');if(value.result)return 'result '+value.result.state.replaceAll('_',' ');if(value.sealRequest)return 'seal '+value.sealRequest.state.replaceAll('_',' ');return ''}
function cardFor(value){const task=value.task,attempt=value.attempt,card=document.createElement('article');card.className='task';card.style.setProperty('--tone',tone(task.state));const head=document.createElement('div');head.className='task-head';const title=document.createElement('h3');title.textContent=task.title;const state=document.createElement('span');state.className='state';state.textContent=task.state.replaceAll('_',' ');head.append(title,state);const progress=progressFor(value),meta=document.createElement('div');meta.className='meta';meta.textContent=[attempt.state,progress,task.baseRef,'rev '+task.revision].filter(Boolean).join(' · ');const actions=document.createElement('div');actions.className='task-actions';const open=document.createElement('a');open.href=attempt.openCodePath;open.textContent='Open session';actions.append(open);if(!value.sealRequest&&['running','input_required'].includes(task.state)&&['admitted','running','input_required'].includes(attempt.state)){const seal=document.createElement('button');seal.type='button';seal.textContent='Seal snapshot';seal.onclick=()=>sealTask(task.id).catch(error=>show(error.message,true));actions.append(seal)}if(!['completed','failed','canceled','cancel_requested'].includes(task.state)){const stop=document.createElement('button');stop.type='button';stop.className='cancel';stop.textContent='Cancel';stop.onclick=()=>cancelTask(task.id).catch(error=>show(error.message,true));actions.append(stop)}card.append(head,meta,actions);return card}
let loading=false;async function load(){if(loading)return;loading=true;try{const response=await fetch('/fern/api/v1/tasks?limit=100',{headers:{Accept:'application/json'}}),data=await response.json().catch(()=>null);if(!response.ok)throw new Error(data?.error?.message||'Task refresh failed');root.replaceChildren();for(const value of data.tasks||[])root.append(cardFor(value));if(!root.children.length)empty('No durable workspace tasks yet.')}finally{loading=false}}
async function submitPending(value,message){submit.disabled=true;show(message);try{await sendPendingSubmission(value);form.reset();document.getElementById('baseRef').value='main';show('Task recorded. Delivery continues in the background.');await load()}catch(error){show(error.message,true)}finally{submit.disabled=false}}
form.addEventListener('submit',event=>{event.preventDefault();try{let value=readPendingSubmission();if(!value){const body=JSON.stringify({title:document.getElementById('title').value,prompt:document.getElementById('prompt').value,baseRef:document.getElementById('baseRef').value});value={version:1,idempotencyKey:crypto.randomUUID(),body};storePendingSubmission(value)}void submitPending(value,'Recording task...')}catch(error){show(error.message,true)}});
document.getElementById('refresh').onclick=()=>load().catch(error=>show(error.message,true));setInterval(()=>{if(document.visibilityState==='visible')load().catch(error=>show(error.message,true))},3000);load().catch(error=>show(error.message,true));try{const pending=readPendingSubmission();if(pending)void submitPending(pending,'Retrying the saved task command...')}catch(error){submit.disabled=true;show(error.message,true)}})();`
