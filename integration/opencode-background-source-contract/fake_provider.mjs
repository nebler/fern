#!/usr/bin/env node
// Zero-cost OpenAI-compatible provider for the source-built background contract.
...
function chatText(response) {
  ...
  line(response, chatChunk({ content: "source contract fake response" }))
...
function chatQuestion(response) {
  ...
  line(response, chatChunk({
    tool_calls: [{
      index: 0,
      id: "call_source_contract_question",
      type: "function",
...
function responsesText(response, body) {
  created(response, body)
  line(response, {
    type: "response.output_item.added",
    sequence_number: 2,
    output_index: 0,
    item: { type: "message", id: "msg_source_contract_provider" },
  })
  line(response, {
    type: "response.output_text.delta",
    sequence_number: 3,
    item_id: "msg_source_contract_provider",
    delta: "source contract fake response",
    logprobs: null,
  })
  line(response, {
...
function responsesQuestion(response, body) {
  ...
  line(response, {
    ...
    item: {
      type: "function_call",
      id: "fc_source_contract_question",
      call_id: "call_source_contract_question",
      name: "question",
      arguments: "",
      status: "in_progress",
    },
  ...
  line(response, {
    ...
    sequence_number: 3,
    output_index: 0,
    item_id: "fc_source_contract_question",
    delta: args,
  })
  line(response, {
...
function hang(response, body, responses) {
  ...
  state.hanging += 1
  const timer = setInterval(() => response.write(": source-contract-heartbeat\n\n"), 100)
  let settled = false
  response.on("close", () => {
    if (settled) return
    settled = true
...
const server = http.createServer((request, response) => {
  ...
  request.on("end", () => {
    ...
    const question = raw.includes("CONTRACT_QUESTION") &&
    ...
    if (!responses) {
      ...
      else chatText(response)
      return
    }
    if (question) responsesQuestion(response, body)
    else responsesText(response, body)
  })
})

server.listen(port, "0.0.0.0", () => console.log(`source fake provider listening on ${port}`))
